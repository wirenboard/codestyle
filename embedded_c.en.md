...
В большинстве случаев нам не важна разрядность переменной. Для счетчиков цикла for, аргументов функций итд нужно использовать типы unsigned для беззнакового или int для знакового типа.

В случае если нам важна разрядность, например в структурах, при реалиции протоколов, буферов итд, нужно использовать явные типы, например uint8_t, uint16_t, uint32_t.
