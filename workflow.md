...
В этом разделе описана работа с ветками _до создания PR_. Работа с ветками, для которых уже создан PR, отличается, смотри следующий раздел.
  ...
  - желательно выдерживать хороший баланс между обьемом коммита и его описанием, не стоит делать комит с `названием добавил режим работы`
  ...
  - если проект - прошивка то последним перед мерджем должен быть коммит с собщением вида change version to: x.x.x с записью в Changelog и изменениями в регистре.
    на данном коммите проект точно должен собираться в сответствующую версию.
...
Общие правила:
  ...
  - последний коммит ветки обязательно должен пройти проверку `continuous-integraion/jenkins/pr-merge`
    (если настроен CI для репозитория);
  - если все изменения в ветке можно уложить в один коммит, то сливать её можно с помощью `Squash and merge`,
    даже если во время ревью были внесены правки или ветка `main` (или `master`) продвинулась;
  - после слияния исходную ветку надо удалить, для этого в Github есть кнопка в интерфейсе;
  - если хочется сохранить несколько коммитов в ветке, идём по инструкции ниже.
