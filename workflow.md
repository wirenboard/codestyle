...
_Удобнее - пользоваться консольным клиентом git!_
...
**Жизненный цикл PR**: Открытие PR => ревью и правки => слияние (Merge) => удаление ветки.
  ...
  - Если в процессе исправлений нужно сделать push (например, чтобы запустить CI) - `git checkout -b tmp/<username>/<что-то_осмысленное>` и push уже её. Так ревьюверам не будут приходить лишние уведомления.
  ...
  - Когда все правки внесены _(и изменения, прилетевшие с ними, проверены)_ - сделать push ветки, из которой открыт PR, и нажать на "re-request review" (сверху в интерфейсе гитхаба; рядом с ревьюверами).
  ...
  - Слияние выполняет автор PR после того, как был получен approve.
  - Последний коммит ветки обязательно должен пройти проверку `continuous-integraion/jenkins/pr-merge`
    (если настроен CI для репозитория). Если Jenkins ругается - не стесняемся читать логи. Не получилось разобраться быстро - дёргаем ответственных за Jenkins.
  ...
  - Сразу удаляй ненужные ветки (через кнопку на гитхабе) - не заставляй Jenkins страдать!
