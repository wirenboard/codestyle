...
-------------------------------------
  ...
  - Команду `@wirenboard/developers` c `Write` доступом для создания PR
    в вашем репозитории. Добавить команду получится только если вы ранее
    выбрали `Public` доступ репозитория.
    В случае приватного репозитория - прийдется добавлять отдельно
    каждого коллегу.
