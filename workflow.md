...
Вариант `Squash and merge` можно выбрать, если в ветке был один коммит. **Нужно** выбрать этот вариант, если так написано
в описании PR (то есть, в описании есть фраза "use Squash and merge"). В остальных случаях **так делать нельзя**!
...
Во втором сценарии возможно два варианта:
 ...
 - если ветка состояла из одного коммита и `main` (или `master`) продвинулась вперёд, то можно сделать `Squash and merge`.
   Это безопасно, потому что CI проверяет PR на корректность с учётом слияния с `main` (или `master`) _(стоит внимательно проверить, будет ли перезапущена
   сборка после обновления `main` (или `master`))_. Это пока **не работает** для прошивок устройств!
