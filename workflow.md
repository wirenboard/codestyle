...
-------------------------------------
  ...
  - Запросить в чате "тыж", чтобы вас добавили в организацию `@wirenboard`,
    и в группу `developers`
  ...
  - Выключить `Wikis`, `Projects`, `Issues` и `Actions` (не используем)
  - Добавить команду `@wirenboard/developers` в коллабораторы c `Maintain`
    доступом. Добавить команду получится только если вы ранее выбрали `Public`
    доступ репозитория. В случае приватного репозитория - прийдется добавлять
    отдельно каждого коллегу.

  5. **Защита веток:**

  Зайти в раздел `Code and automation -> Branches` и добавить
  два отдельных `Branch protection rules`:
  ...
  6. **Создание первого PR:**

  Теперь можно сделать PR - который покажет вы добавляете в мейн коллегам.
