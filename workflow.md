...
**Жизненный цикл PR**:
  ...
  1. Создание. В этот момент нужно добавить как минимум 1 ревьювера. Если сомневаешься, добавь мейнтейнера пакета (указан в `debian/control`) или спроси в чате.
     ...
     а остальных призываем в комментариях. Добавленные в комментариях должны будут своим комментарием написать "approve" или предложить изменения.
  ...
  4. После того, как все ревьюверы поставили approve и пройдены тесты на CI, переходим к подготовке к слиянию.
...
В первом сценарии нужно выполнить слияние, выбрав способ (стрелочка вниз на зелёной кнопке Merge) по инструкции:

  - если в описании PR написано, что нужно выбрать конкретный вариант (фраза вида "use Squash and merge"),
    то нужно обязательно выбрать предложенный вариант;
  - если в ветке несколько коммитов и в описании не написано использовать способ "Squash and merge",
    то нужно выбрать вариант "Create a merge commit";
  - если в ветке был один коммит и в описании нет инструкций, можно использовать любой способ.

Во втором сценарии:

 - если ветка состояла из одного коммита и **в списке Checks есть успешная проверка** `continuous-integration/jenkins/pr-merge`,
   то нужно использовать `Squash and merge`;
 - если в описании PR написано, что можно использовать `Squash and merge` и пройдена проверка `continuous-integration/jenkins/pr-merge`,
   то нужно использовать `Squash and merge`;
...
После принятия PR и слияния все получившиеся ветки, в том числе из закрытых PR, нужно удалить. Удаление ветки
делается нажатием кнопки в интерфейсе Github.

Если для слияния не нужно вносить изменения в историю (делать rebase), все проверки пройдены, получен approve
от всех ревьюверов и в комментариях к PR не указано, кто и как должен вливать PR,
то выполнить слияние может последний ревьювер, поставивший approve.

Если при слиянии нужно сделать какие-то специальные действия, автор PR должен явно указать это в комментариях к PR.
