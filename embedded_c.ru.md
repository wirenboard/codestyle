...
## Именование
### Общее
В embedded C не используется стиль CamelCase, вместо него применяется snake_case. В именах функций и переменных допускаются **только_строчные_буквы**, слова разделяются символом подчеркивания (`_`).

### Файлы и модули
1. При добавлении нового модуля размещайте его в отдельном файле.
...
4. Разделяйте функциональность модулей для повышения переиспользуемости кода. Базовые функции для работы с ИС — в одном файле, задачи — в другом. Например: *mcp230xx.h* содержит драйвер, а *wbio-dio.h* использует его функции.
...
...
```
6. Также в заголовочном файле могут помещаться `static inline` функции, если они, например, используются как заглушки или обертки для других функций.
...
* Компиляторные трюки, невозможные иначе (например, `assert`). Старайтесь ограничивать такими случаями только низкоуровневые библиотеки.

Если в макросе используются выражения, оборачивайте их в `do {} while (0)`. Макросы-функции должны вести себя как функции. Например, `START_TIMER();` вместо `START_TIMER;`. Все макросы именуются ЗАГЛАВНЫМИ буквами.
...
## Ветвления
Запрещается использование тренарных операций, так как они усложняют понимание кода.
